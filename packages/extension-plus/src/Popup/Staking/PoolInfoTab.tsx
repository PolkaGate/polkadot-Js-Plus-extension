...
function PoolInfoTab({ api }: Props): React.ReactElement<Props> {
  ...
  useEffect(() => {
    ...
    ]).then(([maxPoolMembers, maxPoolMembersPerPool, maxPools, minCreateBond, minJoinBond, minNominatorBond]) => {
      setInfo({
        maxPoolMembers: maxPoolMembers.unwrap().toNumber(),
        maxPoolMembersPerPool: maxPoolMembersPerPool.unwrap().toNumber(),
        maxPools: maxPools.unwrap().toNumber(),
