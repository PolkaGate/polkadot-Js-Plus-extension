...
export default function ConfirmStaking({ amount, api, basePool, chain, handlePoolStakingModalClose, nominatedValidators, pool, poolsMembers, proxy, selectedValidators, setConfirmStakingModalOpen, setNewPool, setSelectValidatorsModalOpen, setState, showConfirmStakingModal, staker, stakingConsts, state, validatorsIdentities }: Props): React.ReactElement<Props> {
  ...
  const handleConfirm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (['blocked', 'destroying', 'open'].includes(localState)) {
        ...
        const { block, failureText, fee, status, txHash } = await broadcast(api, poolSetState, params, signer, staker.address, proxy);
  ...
  }, [api, basePool, bondExtra, chain, chilled, claim, decimals, hierarchy, joined, maxUnlockingChunks, nominated, nominatedValidatorsId, password, pool, poolId, poolSetState, poolWithdrawUnbonded, proxy, redeem, selectedValidatorsAccountId, setState, staker.address, state, surAmount, unbonded, unlockingLen]);
